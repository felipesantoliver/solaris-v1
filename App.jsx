...
export default function App() {
  ...
  // Carregar chats e mensagens quando projeto ou chat ativo mudar
  useEffect(() => {
  ...
  const handleSend = async () => {
    ...
    if (!activeChatId) {
      // Se não houver chat ativo, tenta criar um automaticamente
      if (!activeProjectId) return;
      try {
    ...
    try {
      const res = await fetch(`${API_BASE}/messages/chat/${activeChatId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'x-user-id': USER_ID },
  ...
  const ProjectItem = ({ project, isActive }) => (
    <div 
       ...
       className={`flex items-center justify-between p-2.5 -ml-2 rounded-lg cursor-grab active:cursor-grabbing transition-all group/item ${
         isActive ? theme.projectActive : theme.projectHover
            ...
            <div className="flex flex-col gap-1">
                {chatHistory.map((chat) => (
                  <div 
                    ...
                    className={`flex items-center justify-between p-2 -ml-2 rounded-lg cursor-pointer transition-all duration-500 group/chat ${
                      activeChatId === chat.id ? theme.projectActive : theme.projectHover
                    }`}
                  >
