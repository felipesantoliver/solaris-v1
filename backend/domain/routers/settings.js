...
// Migração de guest para usuário logado
...
router.get('/share/:chatId', async (req, res, next) => {
  try {
    ...
    const messages = await getAsync('SELECT role, content, created_at FROM messages WHERE chat_id = $1 ORDER BY created_at ASC', [req.params.chatId]);
