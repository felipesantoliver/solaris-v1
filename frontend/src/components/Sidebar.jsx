...
export const Sidebar = memo(function Sidebar({
  ...
  // Fecha sidebar quando o usuário seleciona item em mobile (tela pequena)
  const closeMobile = () => {
    ...
    <div className={`flex flex-col h-full overflow-hidden transition-all duration-500 ${isMobile || isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
      <div className="px-8 pt-12 pb-6 flex flex-col gap-5 shrink-0">
        ...
        <button
          ...
          className={`flex items-center gap-3 w-full text-left transition-colors ${theme.textPrimary} group`}
        >
        ...
        <button
          onClick={() => {
            if (onNavigate) onNavigate(activeView === 'projects' ? 'chat' : 'projects');
            closeMobile();
          }}
                ...
                <div className="flex items-center gap-3 overflow-hidden flex-1 min-w-0">
                  ...
                  {editingChatTitleId === chat.id ? (
                    <input
                      ...
                      onKeyDown={e => {
                        if (e.key === 'Enter') onConfirmRenameChat(chat.id);
    ...
    <>
      ...
      <aside
        className={`
          ...
          border-r ${theme.border} ${theme.bgAside}
