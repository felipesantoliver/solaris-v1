...
export function MessageInput({
  ...
  const handleKeyDown = (e) => {
    // Em mobile (teclado virtual) Enter não deve enviar — só em desktop
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className={`relative flex items-end border-b ${theme.inputBorder} pb-6 md:pb-8 ${theme.inputFocus} transition-all duration-500`}>
          <textarea
            ...
            className={`flex-1 bg-transparent border-none text-base md:text-lg ${darkMode ? 'text-white placeholder-white/20' : 'text-black placeholder-black/30'} resize-none focus:outline-none py-2 font-light`}
          />
          <button
            onClick={onSend}
        ...
        <div className={`mt-4 md:mt-5 flex justify-between items-center text-[9px] ${theme.textMuted} font-bold tracking-[0.2em] uppercase`}>
          {/* Dica de teclado: oculta em mobile (não há Enter físico) */}
